...
export default class WorkflowStatus extends Command {
  static override description =
    'Get workflow status of an application of a ruleGraph contract'
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      const result: string =
        (await graphContract.methods['getWorkflowStatus']?.(
          flags.applicationId
        ).call()) ?? ''

      this.log(
        'Workflow status of application with applicationId ' +
          flags.applicationId +
          ': ' +
          result
      )

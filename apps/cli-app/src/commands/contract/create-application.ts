...
export default class CreateApplication extends Command {
  static override description =
    'Create a new application in the ruleGraph contract'
  ...
  static override flags = {
    applicationId: Flags.string({
      char: 'i',
      description: 'ID of the application to create',
      required: true
    }),
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      this.log(
        'Creating application with ID: ' +
          flags.applicationId +
          ' on smart contract at address ' +
          flags.contractAddress
      )

      await graphContract.methods['createApplication']?.(
        flags.applicationId
      ).call()

      this.log('Authorize transaction from wallet ' + accounts[0] + '\n')

      const receipt = await graphContract.methods['createApplication']?.(
        flags.applicationId

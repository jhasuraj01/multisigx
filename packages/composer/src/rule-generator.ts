import { AndRule, AtleastRule, BaseRule, LogicRule, OrRule, RuleID, SignRule } from '@jhasuraj01/interface'
...
import { InternalLogicRule } from '@jhasuraj01/interface';

const nameGeneratorConfig: Config = {
  dictionaries: [adjectives, colors, animals, countries, languages, names, starWars],
  separator: '-',
  length: 2,
};
...
export function generate_rule_atleast(_params?: AtleastRuleParams): AtleastRule {
    return {
        ...generate_rule_base(_params),
        type: "ATLEAST",
...
export type SignRuleParams = BaseRuleParams & {
  ...
  internalLogic: SignRule["internalLogic"] // prevent extra properties
}
export function generate_sign_rule(_params: SignRuleParams): SignRule {
    return {
        ...
        internalLogic: _params.internalLogic ?? convert_internal_logic(generate_rule_and())
    }
}